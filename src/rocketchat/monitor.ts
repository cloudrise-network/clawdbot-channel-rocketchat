...
export async function monitorRocketChatProvider(
        ...
        await handleIncomingMessage(
          ...
          async (roomId, isTyping, meta) => {
            ...
            if (botUsername) {
              ...
              // result is usually null/undefined; we log success by reaching here.
              void result;
              return;
            }
  ...
  // Fetch all subscriptions (channels, groups, DMs) and subscribe to each
  const client = createRocketChatClient({ baseUrl, userId, authToken });
  try {
    ...
    // Fall back to subscribing to GENERAL
    await realtime.subscribeToRoom("GENERAL");
  }
